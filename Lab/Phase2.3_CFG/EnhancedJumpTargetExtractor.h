...
#include "../src/vm/disassembly/BasicBlock.h"
...
public:
     ...
     * @brief 提取跳转目标地址（增强版）
     * 
     * @param insn 指令指针（可以是 SimpleInstruction 或 EnhancedSimpleInstruction）
    ...
    static uint64_t extract_jump_target(
        ...
        // 尝试转换为增强版本（通过检查地址范围）
        // 注意：这里假设如果是指针相同，就是 EnhancedSimpleInstruction
        const EnhancedSimpleInstruction* enhanced_insn = 
            static_cast<const EnhancedSimpleInstruction*>(insn);
        
        if (enhanced_insn->has_capstone_data && enhanced_insn->capstone_insn) {
            // ✅ 使用 Capstone 详细数据
            return extract_from_capstone_detail(enhanced_insn->capstone_insn, current_addr);
        } else {
            // ⚠️ 回退到字符串解析（兼容性）
